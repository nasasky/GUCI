...
class CategoryController extends PublicController {
    ...
    public function index(){
    	$list = M('category')->where('tid=1')->field('id,tid,name,bz_1,bz_4')->select();
        $catList = M('category')->where('tid='.intval($list[0]['id']))->field('id,name,bz_1,bz_4')->order("sort desc")->select();
