...
class ProductController extends PublicController {
	...
	public function col(){
		...
		if ($res) {
			echo json_encode(array('status'=>1,"err"=>$msg));
			exit();
		}else{
			echo json_encode(array('status'=>0,'err'=>'网络错误..'));
			exit();
	...
	public function getcol(){
		...
		$product_sc=M("product_sc");
		$product=M("product");
		$proarr=array();
		$col=$product_sc->where("status=1 AND uid=".$uid)->field("pid")->select();
		foreach ($col as $k => $v) {
			$pro=$product->where("id=".$v['pid'])->find();
			$pro['photo_x']=__DATAURL__.$pro['photo_x'];
			$proarr[$k]=$pro;
		}
		echo json_encode(array("status"=>1,"err"=>$proarr));
	 ...
	 * @return [type] [description]
	 */
	public function cancalcol(){
		$uid=I("post.uid");
		$proid=I("post.proid");
		if($uid && $proid){
			$re=M("product_sc")->where("uid=$uid AND pid=$proid")->delete();
			if($re>0){
