...
class UserController extends PublicController{
	...
	public function user_del(){
		if(IS_AJAX){
			...
			$re=M("user")->where("id=$id")->setField("del",1);
			if($re){
				$this->ajaxReturn(1);
			}else{
				$this->ajaxReturn(0);
	...
	public function normal(){
		$list=M("user")->where("level<10 AND del=0 AND qx=6")->select();
		$this->assign("list",$list);
		$this->display();
	}
	public function vip(){
		$list=M("user")->where("level>=10 AND level<20  AND del=0 AND qx=6")->select();
		$this->assign("list",$list);
		$this->display();
	}
	public function gold(){
		$list=M("user")->where("level>=20 AND level<30 AND del=0 AND qx=6")->select();
		$this->assign("list",$list);
		$this->display();
	}
	public function king(){
		$list=M("user")->where("level>=30 AND del=0 AND qx=6")->select();
		$this->assign("list",$list);
		$this->display();
	}
}